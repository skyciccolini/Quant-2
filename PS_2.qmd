...
PP <- ulam(
  alist(
    ht99mm ~ dnorm(mu, sigma),
    mu ~ dnorm(200, 10), ### how are mine compared to theirs?
...
PP <- ulam(
  alist(
    ...
    mu ~ dnorm(5, 5), ### how are mine compared to theirs?
    sigma ~ dhalfnorm(0, 1)
  ),
  data = lac,
  sample_prior = TRUE
...
> ### something is funky with my posteriors
...
```{r}
### but make this the updated model
PP <- ulam(
  alist(
    loght99mm ~ dnorm(mu, sigma),
    mu ~ dnorm(200, 10),
...
### run this line by line first because its probably wrong
post_sub <- post_lac |> 
  slice_sample(n = 25) |> 
  mutate(Iteration = numeric(1:25)) |> 
