...
library(pacman)
p_load(tidyverse, rethinking, stan, readxl)

fir <- read_xlsx("raw_data/Silver_fir.xlsx")

fir$ht99mm <- as.numeric(fir$ht99mm)

fir <- fir |> 
  filter(population == "CHE" | population == "LAC") |> 
...
```{r}
che <- fir |> 
  filter(population == "CHE") |> 
  as_tibble()

lac <- fir |> 
  filter(population == "LAC") |> 
  as_tibble()
...
che_mu <- mean(che$ht99mm)

che_med <- median(che$ht99mm)

ggplot() +
  geom_histogram(data = che, aes(x = ht99mm)) +
  geom_vline(xintercept = che_mu) +
  geom_vline(xintercept = che_med, color = "red")
```
...
ulam(
  alist(
    ht99mm ~ dnorm(mu, sigma), ## distribution for the outcome
    mu ~ dnorm(100, 10), ## model statement
    sigma ~ dhalfnorm(0, 5)
  ),
  data = che
)
...
## extract the mu and sigma samples from above
...
> I think its pretty good. There is probably a bit too much density at the smaller values, but I'm happier with how it captures the larger values.
...
> rhat is 1 so that is ok. n_eff also seems to be large enough.
...
> mu: mean 201.867, HDI 196.6565 - 206.7525
sigma: mean 53.31487, HDI 51.96441 - 54.71661

How do the means and HDIs of these posterior estimates compare to the values you used for the prior?

> For mu, the mean is very close to the prior and my prior falls within the HDI. For sigma, the mean is a bit higher than the prior and my prior is slightly lower than the HDI.
...
```{r}
lac_mu <- mean(lac$ht99mm)

lac_med <- median(lac$ht99mm)

ggplot() +
  geom_histogram(data = lac, aes(x = ht99mm)) +
  geom_vline(xintercept = lac_mu) +
  geom_vline(xintercept = lac_med, color = "red")

```

Describe the shape of the histogram and the relationship between mean and median.

> The histogram is skewed to the right a bit.

Do the log-transformation here:

```{r}
lac$loght99mm <- log(lac$ht99mm)
...
PP <- ulam(
  alist(
    loght99mm ~ dnorm(mu, sigma),
    mu ~ dnorm(5, 0.25), 
    sigma ~ dhalfnorm(0, 0.25)
  ),
  data = lac,
  sample_prior = TRUE
)
...
PP_sim <- purrr::map(
  ...
  .f = function(ii) {
    tibble(ID = ii,
           ht99mm_sim = seq(0, 10, length.out = 20),
           density = dnorm(ht99mm_sim,
                           mean = post$mu[ii],
                           sd = post$sigma[ii]))
  }) |> 
...
p_obs <- ggplot(lac, aes(loght99mm)) +
  geom_histogram(aes(y = after_stat(density)),
                 fill = "#2D5DA1", bins = 30) +
  xlim(c(0, 10))
p_pp <- ggplot(PP_sim, aes(x = ht99mm_sim, y = density, group = ID)) +
...
> I tried so many priors, but I ended up using (5, 0.25) for mu and (0, 0.25) for sigma.
...
> Yes, the posterior distribution looks good.
...
> The rhat is good at 1, the n_eff is in the 100s which is ok.

What are the means and 89% HDIs for `mu` and `sigma`? You can use the `HPDI()` function from rethinking.

```{r}
post <- extract.samples(PP) |> 
  as.data.frame()
...
> mu: mean 4.991544, HPDI 4.582932 5.353870 
sigma: mean 0.1966383, HPDI 0.0002266208 0.4001367100 

How do the means and HDIs of these posterior estimates compare to the values you used for the prior?

> The HPDIs capture both of my priors for mu and sigma and the means are very similar as well.
...
> Changing the mu and the sigma in the prior changes that mu and the sigma in the posterior.
...
post_lac <- extract.samples(PP) |> as.data.frame()

post_sub <- post_lac |> 
  slice_sample(n = 25) |> 
  mutate(Iteration = 1:25) |> 
  pivot_longer(cols = mu:sigma,
                names_to = "Parameter",
                values_to = "Estimate")

ggplot(data = post_sub, aes(x = Iteration, y = Estimate)) +
  geom_point() +
...
> The samples are pretty scattered around 5.05 for mu and 0.37 for sigma.
...
ggplot() +
  geom_point(data = post_lac, aes(sigma, mu))

```

What do you observe in the scatter plot?

> The points are all condensed around 0.37, 5.05 and then they kind of spread out equally from there.
...
post_mcmc <- as.mcmc(post_lac)

autocorr.diag(post_mcmc)
