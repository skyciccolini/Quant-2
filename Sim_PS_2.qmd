...
salmon_jack <- tibble(ps = pseudovals, freq = allele_freq) ### this seems wrong
...
> I have concerns. All of my pseudovalues are the same
...
```{r}
#| echo: true
mu_ps <- mean(pseudovals$ps)
ci_ps <- c(mu_ps + qt(0.975, length(salmon$ID) - 1) * 
             sd(pseudovals$ps) / sqrt(length(salmon$ID)),
...
salmon_full <- rep(salmon$ID, salmon$Clock_Allele) 

freq <- function(salmon,indicies) {
  frequency <- sum(salmon[indicies])/length(salmon$ID)
  return(frequency)
}
