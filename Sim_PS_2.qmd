...
small <- read_csv("raw_data/small_data.csv")
salmon <- read.csv("raw_data/salmon_pop.csv")
...
mod <- broom::tidy(summary(lm(y ~ Group, data = small)))

t_obs <- mod$estimate[2] 
...
for (ii in 2:niter) { 
  small.s <- small[,c("Group","y")] 
  small.s$Group <- sample(small.s$Group) 
  mod <- broom::tidy(summary(lm(y ~ Group, data = small.s))) 
  rand.out[ii,] <- mod$estimate[2] 
  }


sum(abs(rand.out$Tstat) >= abs(rand.out$Tstat[1]))/nrow(rand.out) ### I think I may have done this wrong
...
rand.out <- map_dbl(.x = iters, 
        .f = function(ii) rand(ii, small = small))
...
unique(salmon$Clock_Allele)

freq_c <- sum(salmon$Clock_Allele)/length(salmon$ID)
...
pseudovals <- numeric(length=length(salmon$ID))
allele_freq <- numeric(length = length(salmon$ID))

for(ii in 1:length(salmon$ID)) {
  # delete line ii
  salmon.p <- salmon |>
    filter(ID != ID[ii])
  
  # estimate frequency
  H2.p <- sum(salmon.p$Clock_Allele)/length(salmon.p$ID)
  
  # save allele freq
  allele_freq[ii] <- H2.p
  
  #calculate pseudovalue
  pseudovals[ii] <- length(salmon$ID) * H2.0 - (length(salmon$ID) - 1) * H2.p
}

salmon_jack <- tibble(ps = pseudovals, freq = allele_freq)
...
> The pseudovalues are all 0 or 1 which makes sense since thats what the original allele counts were.I don't think I have any concerns.
...
```{r}
#| echo: true
mu_ps <- mean(salmon_jack$ps)
ci_ps <- c(mu_ps + qt(0.975, length(salmon$ID) - 1) * 
             sd(salmon_jack$ps) / sqrt(length(salmon$ID)),
           mu_ps - qt(0.975, length(salmon$ID) - 1) * 
             sd(salmon_jack$ps) / sqrt(length(salmon$ID)))
...
freq <- function(salmon,indicies) {
  frequency <- sum(salmon$Clock_Allele[indicies])/length(salmon$ID)
  return(frequency)
}

salmon_bs <- boot(data = salmon_full,
                statistic = freq,
                R = 1000,
                stype = "i")
...
> They seem normally distributed around 1.8, which makes sense since that is the mean frequency for the clock allele. I have some concerns that the dataset is good representation of the true distribution.
...
freq <- function(salmon,indicies) {
  frequency <- sum(salmon$Clock_Allele_bi[indicies])/length(salmon$ID)
  return(frequency)
}
...
ci_results <- tibble(
  jack_lwr = numeric(niter),
  jack_upr = numeric(niter),
  boot_lwr = numeric(niter),
  boot_upr = numeric(niter)
)

for (i in 1:niter) {
  # generate rand numbers
  bi <- rbinom(n = n, prob = freq_c, size = 1)

  bi_allele <- tibble(ID = salmon$ID,
                    Clock_Allele_bi = bi)
  # jackknife
  H2.0 <- freq_c

  pseudovals <- numeric(length = length(bi_allele$ID))
  allele_freq <- numeric(length = length(bi_allele$ID))

  for(ii in 1:length(bi_allele$ID)) {
    # delete line ii
    bi_allele.p <- bi_allele |>
      filter(ID != ID[ii])
  
    # estimate frequency
    H2.p <- sum(bi_allele.p$Clock_Allele_bi)/length(bi_allele.p$ID)
  
    # save allele freq
    allele_freq[ii] <- H2.p
  
    #calculate pseudovalue
    pseudovals[ii] <- length(bi_allele$ID) * H2.0 - (length(bi_allele$ID) - 1) * H2.p
  }

  bi_salmon_jack <- tibble(ps = pseudovals, freq = allele_freq)

  mu_ps <- mean(bi_salmon_jack$ps)

  ci_ps <- c(mu_ps + qt(0.975, length(salmon$ID) - 1) * 
             sd(bi_salmon_jack$ps) / sqrt(length(salmon$ID)),
           mu_ps - qt(0.975, length(salmon$ID) - 1) * 
             sd(bi_salmon_jack$ps) / sqrt(length(salmon$ID)))
  
  # boot strap

  bi_salmon_bs <- boot(data = bi_allele,
                statistic = freq,
                R = n,
                stype = "i")

  ci_bs <- boot.ci(bi_salmon_bs, type = "perc")$percent[4:5]
  
  # store cis in tibble
  ci_results$jack_lwr[i] <- ci_ps[1]
  ci_results$jack_upr[i] <- ci_ps[2]
  ci_results$boot_lwr[i] <- ci_bs[1]
  ci_results$boot_upr[i] <- ci_ps[2]


}


# percent that ci includes true freq
...
ci_true$jack <- ifelse(ci_results$jack_lwr <= freq_c & freq_c <= ci_results$jack_upr, 1, 0)

sum(ci_true$jack)/length(ci_true)

ci_true$boot <- ifelse(ci_results$boot_lwr <= freq_c & freq_c <= ci_results$boot_upr, 1, 0)

sum(ci_true$boot)/length(ci_true)
...
> I think the boot strap works ok, but the jacknife doesn't work very well at all. The boot strap ci contained the true freq about 25% of the time, but the jackknife rarely contained it.

What values should further simulations change to test the generality of these methods? (Try some out if you wish)

> I would change up the allele frequencies and the number of samples in the original data set. I think with a larger dataset to start with, the methods may work better.
...
stick <- abdData::SticklebackPlates

ggplot() +
  geom_histogram(data = stick, aes(plates)) +
  facet_grid(stick$genotype)
...
# find observed diffs in mean and median
mu_mm_MM <- stick |> 
  filter(genotype == "mm") |> 
  summarize(mu = mean(plates)) |> 
  pull(mu) -
   stick |> 
  filter(genotype == "MM") |> 
  summarize(mu = mean(plates)) |> 
...
fm_lm <- anova(lm(plates ~ genotype, data = stick))

obsF <- fm_lm$`F value`[1]

obs <- tibble(Fstat = obsF, mu_mm_MM = mu_mm_MM, 
              mu_mm_Mm = mu_mm_Mm, mu_Mm_MM = mu_Mm_MM,
              med_mm_Mm = med_mm_Mm, 
              med_mm_MM = med_mm_MM,
              med_Mm_MM = med_Mm_MM)
...
set.seed(6445287)
niter <- 10000
rand.out <- tibble(Fstat = rep(NA,niter), 
                   mu_mm_MM = rep(NA,niter), 
              mu_mm_Mm = rep(NA,niter), 
              mu_Mm_MM = rep(NA,niter),
              med_mm_Mm = rep(NA,niter), 
              med_mm_MM = rep(NA,niter),
              med_Mm_MM = rep(NA,niter))
...
for(ii in 2:niter) {
  ...
  stick.s$plates <- sample(stick$plates)
...
fm_lm <- anova(lm(plates ~ genotype, data = stick.s))

obsF <- fm_lm$`F value`[1]

rand.out[ii, ] <- tibble(Fstat = obsF, mu_mm_MM = mu_mm_MM, 
              mu_mm_Mm = mu_mm_Mm, mu_Mm_MM = mu_Mm_MM,
              med_mm_Mm = med_mm_Mm, 
              med_mm_MM = med_mm_MM,
              med_Mm_MM = med_Mm_MM)
...
ggplot() +
  geom_histogram(data = rand.out, aes(mu_mm_Mm))

# pivot
rand.out_long <- pivot_longer(rand.out,
                    cols = everything(),
                    names_to = "stat",
                    values_to = "value")
...
# add obs back in
rnad.out_obs <- left_join(rand.out_long, obs_vals, by = "stat")

ggplot(data = rnad.out_obs) +
...
> The mean seems to be the most appropriate test statistic.
...
set.seed(6445287)
niter <- 10000
rand.out <- tibble(med_mm_Mm = rep(NA,niter), 
              med_mm_MM = rep(NA,niter),
              med_Mm_MM = rep(NA,niter),
              thresh_diff = rep(NA,niter))
...
thresh_diff <- sum(c(abs(med_mm_Mm), abs(med_mm_MM), abs(med_Mm_MM)) > thresh)

rand.out[ii, ] <- tibble(med_mm_Mm = med_mm_Mm, 
              med_mm_MM = med_mm_MM,
              med_Mm_MM = med_Mm_MM,
...
ggplot(rand.out) +
  geom_histogram(aes(thresh_diff), bins = 10)


mean(rand.out$thresh_diff)

## expected number of false positives 0.0144
```
