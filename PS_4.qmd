...
```{r}
### check this code chunk because its probably wrong
PP <- ulam(
  alist(
    Milk_energy ~ dnorm(mu, sigma),
    mu <- b0 + b1 * Fat + b2 * Lactose,
    b0 ~ dnorm(0, 0.2),
    b1 ~ dnorm(0, 0.2),
    b2 ~ dnorm(0, 0.2),
    sigma ~ dhalfnorm(0, 1)
...
ggplot() +
  geom_point(data = milk,
             aes(Fat, Milk_engery, color = factor(Lactose)),
             size = 4) +
    # scale_colour_manual(values = c("red", "blue"),
    #                     name = "Lactose",
    #                     labels = c("Active", "Ambush")) +
  geom_abline(data = pp_dist, aes(slope = b1, intercept = b0),
              color = "red", alpha = 0.25) +
  geom_abline(data = pp_dist, aes(slope = b2, intercept = b0),
...
# make it brms
priors <- c(prior(normal(0, 0.2), class = b),
            prior(normal(0, 0.2), class = b, coef = Fat),
            prior(normal(0, 0.2), class = b, coef = Lactose),
            prior(normal(0, 1), class = sigma)
            )

fm_brm <- brm(Milk_energy ~ Fat + Lactose, ### think i did this wrong
              data = milk,
              prior = priors,
              iter = 5e3)
```
...
```{r}
bayesplot::pp_check(fm, fun = "dens_overlay", yrep[1:50, ]) ### cant get this to work
```
...
p_pred <- posterior_epred(fm_brms, newdata = new_data) ### check this
