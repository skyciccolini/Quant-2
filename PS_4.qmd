...
```{r}
priors <- c(prior(normal(4, 2), class = b, lb = 0),
...
get_prior(logSurvival ~ Mismatch - 1, data = heart)
...
\begin{align}
  \mathrm{logSurvival} & \sim Normal(\mu, \sigma) \\
  \mu & = b[\mathrm{Mismatch}] \\
  b[\mathrm{Mismatch}] & \sim Normal(4, 2) \\
  \sigma & \sim Normal (0, 3) \\
\end{align}
```
### Sampling
...
powerscale_plot_dens(
  powerscale_sequence(fm),
  quantity = c("mean", "sd"),
  variable = "b_MismatchHigh")

powerscale_plot_dens(
  powerscale_sequence(fm),
  quantity = c("mean", "sd"),
  variable = "b_MismatchLow")

powerscale_plot_dens(
  powerscale_sequence(fm),
  quantity = c("mean", "sd"),
  variable = "b_MismatchMedium")
...
powerscale_plot_quantities(
  powerscale_sequence(fm),
  quantity = c("mean", "sd"),
  variable = c("b_MismatchHigh", "b_MismatchLow", "b_MismatchMedium", "sigma"))
```
...
fm |>
  spread_rvars(b_MismatchHigh, b_MismatchLow, b_MismatchMedium,sigma)
...
post_contrast <- post |> 
  mutate(`Medium vs. Low` = b_MismatchMedium - b_MismatchLow,
         `High vs. Low` = b_MismatchHigh - b_MismatchLow,
         .keep = "none")
...
bird <- readxl::read_xlsx("raw_data/Bird_Plasma.xlsx") |> 
  mutate(Treatment = as.factor(factor(Treatment,
...
```{r}
priors <- c(prior(normal(20,5), class = b, lb = 0),
                 prior(normal(2, 2), class = sigma))
...
get_prior(Calcium ~ Sex_Trt -1, data = bird)
...
powerscale_plot_dens(
  powerscale_sequence(fm),
  quantity = c("mean", "sd"),
  variable = "b_Sex_TrtFemale_Hormone")

powerscale_plot_dens(
  powerscale_sequence(fm),
  quantity = c("mean", "sd"),
  variable = "b_Sex_TrtFemale_None")

powerscale_plot_dens(
  powerscale_sequence(fm),
  quantity = c("mean", "sd"),
  variable = "b_Sex_TrtMale_Hormone")

powerscale_plot_dens(
  powerscale_sequence(fm),
  quantity = c("mean", "sd"),
  variable = "b_Sex_TrtMale_None")
...
powerscale_plot_quantities(
  powerscale_sequence(fm),
  quantity = c("mean", "sd"),
  variable = c("b_Sex_TrtFemale_Hormone", "b_Sex_TrtFemale_None", "b_Sex_TrtMale_Hormone", "b_Sex_TrtMale_None", "sigma"))
...
post_contrast <- post |> 
  mutate(`Female Hormone vs. Control` = 
           b_Sex_TrtFemale_Hormone - b_Sex_TrtFemale_None,
...
post_contrast |>
  ...
  stat_slab(alpha = 0.5) +
  scale_fill_manual(values = c("red", "blue"), name = "Contrast") +
...
```{r}
milk <- read_xlsx("raw_data/Milk.xlsx") |> 
...
ggplot(milk, aes(Fat, Milk_energy)) +
  geom_point()

ggplot(milk, aes(Lactose, Milk_energy)) +
  geom_point()
...
```{r}
priors <- c(prior(normal(0.2, 0.05), class = b),
            prior(normal(0, 1), class = sigma))
...
get_prior(Milk_energy ~ Fat + Lactose, data = milk)
...
powerscale_plot_quantities(
  powerscale_sequence(fm),
  quantity = c("mean", "sd"),
  variable = c("b_Fat", "b_Lactose", "b_Intercept", "sigma"))
...
p_pred <- posterior_epred(fm, newdata = new_data) 
...
ggplot() +
  geom_ribbon(
    ...
    alpha = 0.25) +
  geom_line(data = new_data, aes(x = Fat, y = Q50, color = Lactose)) +
  facet_grid(. ~ Lactose) +
...
post <- fm |>
  ...
  mutate(
    `Fat vs. Intercept` = b_Fat - b_Intercept,
    `Lactose vs. Intercept` = b_Lactose - b_Intercept,
    .keep = "none")
...
post |>
  ...
  stat_slab(alpha = 0.5) +
  scale_fill_manual(values = c("red", "blue"), name = "Contrast") +
  labs(x = "Fat/Lactose vs Control", y = "Density")
