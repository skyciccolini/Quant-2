...
lizard <- read.csv("raw_data/HornedLizards.csv") |> 
  drop_na()
...
```{r}
ggplot(data = lizard) +
  geom_histogram(aes(x = horn_length)) +
  facet_grid("group")
...
> The alive lizards seem to have a larger average horn length. They also seem to have more samples than the dead lizards.
...
lizard |> 
  group_by(group) |> 
  summarise(no = length(horn_length))

# check mean
lizard |> 
  group_by(group) |> 
  summarise(mean = mean(horn_length))

# check variances
lizard |> 
  group_by(group) |> 
  summarise(var = var(horn_length))
...
```{r}
# make group an integer
lizard$group <- as.integer(factor(lizard$group))
...
\begin{align}
  \mathrm{horn\_length} & \sim Normal(\mu, \sigma) \\
  \mu & = b[\mathrm{group}] \\
  b[\mathrm{group}] & \sim Normal(24, 7.5) \\
...
> There were some warning about chain 1 and how the model may be ill-conditioned or misspecified.
...
> The rhat is good at 1 and ess_bulk is in the thousands which is good.
...
> Both of these visual checks seem good to me. The traceplot for the two groups look normal. The sigma traceplot looks a little funky.
...
ggplot(data = pp_dist_long, aes(horn_length, color = "group", linetype = PP)) +
  geom_density() +
  facet_grid(group ~ .) +
  cowplot::theme_cowplot() +
  labs(x = "Horn Length (mm)", y = "Count") +
...
### can't get linetype to work
ggplot(linetype = "PP") +
  geom_density(data = lizard, aes(horn_length, color = group)) +
  geom_density(data = pp_dist2_long, aes(horn_length, color = group)) +
  facet_grid(group~.) +
  scale_linetype_manual(values = c("Observed" = "solid", "Posterior" = "dashed"))

```

How does the posterior for each group compare to the observed data?
> The posterior adequately covers the same range as the observed data.
...
lizard |> 
  group_by(group) |> 
  summarise(mean = mean(lizard$horn_length))

pp_dist2_long |> 
  group_by(group) |> 
  summarise(mean = mean(pp_dist2_long$horn_length))


```

How do they compare?
> The means are very close together (within 0.2). ### double check
...
```{r}
ggplot()+
  geom_density(data = diff, aes(diff))

```

What is your interpretation of the plot?
> There seems to be a larger mean horn length for the alive lizards than the dead lizards.
...
```{r}
HPDI(diff$diff, prob = 0.89)


```

Does the interval agree with your assessment of the plot?
> The HPDI is pretty wide and crosses 0, so no? ###
...
[^2]: Tomkins, J.L. and G.S. Brown. 2004. Population density drives the local evolution of a threshold dimorphism. *Nature* 431: 1099-1103.
...
```{r}
earwig <- read_csv("raw_data/Earwigs.csv")

ggplot(data = earwig, aes(Density, Proportion_forceps)) +
  geom_point()
```
...
```{r}
# no intercept because it doesn't make sense for a population to have a density of 0
PP <- ulam(
  alist(
    ...
    b0 == 0,
    b1 ~ dnorm(1, 5),
    sigma ~ dhalfnorm(0, 10) ### check priors
  ),
  data = earwig,
  sample_prior = TRUE
)
...
```{r}
jetlag <- read_csv("raw_data/JetLag.csv")

ggplot(data = jetlag) +
  geom_density(aes(Shift)) +
  facet_grid(Treatment~.)

jetlag$Treatment <- as.integer(factor(jetlag$Treatment))
...
pp_dist |> 
  pivot_longer(cols = everything(),
               names_to = "Treatment", values_to = "Shift") |> 
  ggplot(aes(Shift, fill = Treatment)) +
  geom_histogram(bins = 30) +
  scale_fill_manual(values = c("tomato", "steelblue", "pink")) +
  facet_grid(Treatment ~ .) +
  cowplot::theme_cowplot() +
  labs(x = "Shift", y = "Count") +
  theme(legend.position = "none")
```
...
\begin{align}
  \mathrm{Shift} & \sim Normal(\mu, \sigma) \\
  \mu & = b[\mathrm{Treatment}] \\
  b[\mathrm{Treatment}] & \sim Normal(0, 5) \\
  \sigma & \sim HalfNormal(0, 5) \\
\end{align}
```
...
pp_dist_long <- pp_dist3 |> 
  pivot_longer(cols = -PP, names_to = "Treatment", values_to = "Shift")

pp_dist_long$PP <- factor(pp_dist_long$PP, levels = c("Prior", "Posterior"))

### still only getting one line
ggplot(data = pp_dist_long, aes(Shift, color = "Treatment", linetype = PP)) +
  geom_density() +
  facet_grid(Treatment ~ .) +
  cowplot::theme_cowplot() +
  labs(x = "Shift", y = "Count") +
...
jetlag |> 
  group_by(Treatment) |> 
  summarise(mean = mean(jetlag$Shift))

pp_dist2_long |> 
  group_by(Treatment) |> 
  summarise(mean = mean(pp_dist2_long$Shift))

knee_control <- pp_dist2 |> 
  mutate(diff = knee - control)

ggplot()+
  geom_density(data = knee_control, aes(diff))

HPDI(knee_control$diff, prob = 0.89)

eye_control <- pp_dist2 |> 
  mutate(diff = eyes - control)

ggplot()+
  geom_density(data = eye_control, aes(diff))

HPDI(eye_control$diff, prob = 0.89)
